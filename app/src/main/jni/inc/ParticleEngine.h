...
class ParticleEngine
{
    ...
    ParticleEngine(uint particleNumber, bool isAuto = false, uint threadNb = 0);
    ~ParticleEngine();
...
//        m_randomSpeed = -RANDOM_SPEED;
    	int temp;
    	while ((temp = Utils::myRand(0, mFunctionsArraySize)) == mFunctionCursor);
    	// Always eight gravity to gravity
    	if (mFunctionCursor == 3)
    		temp = 1;
    	if (mFunctionCursor == 1)
    		temp = 2;
    	mFunctionCursor = temp;
//    	if (++mFunctionCursor >= mFunctionsArraySize)
//    		mFunctionCursor = 0;
    	mParticleStep = mFunctionsArray[mFunctionCursor];
    ...
    inline long particleNumber()
    {
        return m_part_nbr;
    }

    inline void setParticleNumber(long pnumber)
    {
    ...
    inline long maxParticleNumber()
    {
        return m_max_part_nbr;
    }

    inline void initParticle(uint part)
    {
    	(*this.*mParticleInit)(part);
    }
    ...
    private:
    typedef void (ParticleEngine::*ParticleInitFunction)(uint pid);
    typedef void (ParticleEngine::*ParticleStepFunction)(int start, int end);

    void    moveWave();
    bool    initParticles();
    void    initParticlePlatform(uint idx);
    void 	initParticleRandom(uint idx);
    void 	allocFailed(int size);
    void	initFunctions();
    
    void 	stepEnginePlatform(int start, int end);
    void 	stepEngineGravity(int start, int end);
    void	stepEngineLorrenz(int start, int end);
    void	stepEngineEightGravity(int start, int end);
    ...
    ParticleStepFunction* mFunctionsArray;
    int 				mFunctionsArraySize;
    ParticleInitFunction mParticleInit;
    ParticleStepFunction mParticleStep;
    int mFunctionCursor;

...
void Utils::onFPSUpdate(JNIEnv *env, float cpu, float gpu, jlong particleNumber)
{
	jclass jcaller = env->FindClass("com/valxp/particles/JavaCaller");
	if (jcaller == 0)
	{
	...
	jmethodID loader = env->GetStaticMethodID(jcaller, "onFPSUpdate", "(FFJ)V");
	if (loader == 0)
	{
		LOGE("Cannot locate static method onFPSUpdate\n");
		exit(EXIT_FAILURE);
	}
	env->CallStaticVoidMethod(jcaller, loader, cpu, gpu, particleNumber);
}

...
bool            GlApp::setupGraphics(int w, int h)
{
	...
	checkGlError(__FILE__, __LINE__);
	m_model.setidentity();
	m_projection.setProjection(0.1, 100, 45, ((float)w)/((float)h));
...
void            GlApp::countFPS()
{
	...
	if (current - m_last > 1)
	{
		...
		if (engineFrameNb != 0 && gpuFrameNb != 0)
		{
			...
			if (!m_engine->paused() && m_stepCount > 2 && m_engine->isAuto())
				m_engine->setParticleNumber(m_engine->particleNumber() + FPS_to_particles(GPUFPS, CPUFPS, m_engine->particleNumber()));
#ifdef ANDROID
			Utils::onFPSUpdate(m_jenv, CPUFPS, GPUFPS, m_engine->particleNumber());
#endif
		}
...
long 	GlApp::FPS_to_particles(float gpu, float cpu, long particleNumber)
{
	float fps = fmin(gpu, cpu);
	if (fps > 60)
		fps = 60;
	if (fps < 20 && particleNumber > 100)
	{
		float delta = 1 - (fps / 20);
		return -particleNumber * delta;
	} else if (fps > 30)
	{
		float delta = (fps / 30) / 10;
		return delta * particleNumber;
	}
}

...
// TODO: Multisampling : https://code.google.com/p/gdc2011-android-opengl/source/browse/trunk/src/com/example/gdc11/MultisampleConfigChooser.java
...
public class GLView extends GLSurfaceView {
    ...
    public GLView(Context context, long pnumber2, float psize, boolean motionBlur, boolean autoGenerate) {
        super(context);
        mContext = context;
        GLView.psize = psize;
        GLView.pnumber = pnumber2;
        GLView.motionBlur = motionBlur;
        GLView.autoGenerate = autoGenerate;
        init(false, 0, 0);
        mScaleDetector = new ScaleGestureDetector(context, new ScaleListener());
        setOnClickListener(l);
        try {
            Method method = GLSurfaceView.class.getDeclaredMethod("setPreserveEGLContextOnPause", boolean.class);
            method.invoke(this, true);
        } catch (Exception e) {
            e.printStackTrace();
            Log.d(TAG, "Cannot keep opengl context... unsupported android version");
        }
    }
    ...
    private class Renderer implements GLSurfaceView.Renderer {
        ...
        public void onSurfaceChanged(GL10 gl, int width, int height) {
            ...
            ParticlesCPP.init(width, height, particles, psize, motionBlur, autoGenerate);
    ...
    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            mScaleChanged = true;
            Log.d(TAG, "Factor : " + detector.getScaleFactor() + " span " + detector.getCurrentSpan());
            mScaleFactor += (detector.getScaleFactor() - 1) * 10;

            // Don't let the object get too small or too large.
            //mScaleFactor = Math.max(0.05f, Math.min(mScaleFactor, 1.5f));

            ParticlesCPP.setZoom(mScaleFactor);
